...
Peptide_longer <- readRDS("C:/Users/maxim/OneDrive - Montefiore Medicine/Shechter/Processed_Data/Tryp_02/MMA_tidy_TheoMW_03.RDS")
...
##build data
#example <- UbiLength
experimental_design <- read.csv('Trypsin/MMA_Tryp_tidy_peptides_TheoMW_ExpDesign.csv', header =T)
...
data_se <- make_se(data_unique, grep("MMA", colnames(data_unique)), experimental_design)
...
#data_diff <- test_diff(data_imp, type = "control", control = "Control_MMA")

# Test all possible comparisons of samples
#data_diff <- test_diff(data_imp, type = "all")
# Test manually defined comparisons
data_diff <- test_diff(data_imp, type = "manual", 
                              test = c("GSK591_MMA_vs_DMSO_MMA", 'MS023_MMA_vs_DMSO_MMA', 'MS023_MMA_vs_GSK591_MMA'))
...
plot_volcano(dep, contrast = "GSK591_MMA_vs_DMSO_MMA", label_size = 2, add_names = TRUE)
plot_volcano(dep, contrast = "MS023_MMA_vs_DMSO_MMA", label_size = 2, add_names = TRUE)
plot_volcano(dep, contrast = "MS023_MMA_vs_GSK591_MMA", label_size = 2, add_names = TRUE)
# Plot a barplot for USP15 and IKBKG
plot_single(dep, proteins = c("CHTOP.8"), type ='centered')
plot_single(dep, proteins = c("CHTOP.33"), type ='centered')
plot_single(dep, proteins = c("CHTOP.43"), type ='centered')
plot_single(dep, proteins = c("CHTOP.44"), type ='centered')
plot_single(dep, proteins = c("CHTOP.48"), type ='centered')
plot_single(dep, proteins = c("CHTOP.49"), type ='centered')
...
#write.csv(df_wide, 'C:\\Users\\maxim\\OneDrive\\Shechter\\DEP\\Notebooks\\Trypsin\\perResidue\\Trypsin_MMA_Peptide_Volcano.csv')

#plot pvals
hist(data_results$GSK591_MMA_vs_DMSO_MMA_p.val)
hist(data_results$MS023_MMA_vs_DMSO_MMA_p.val)
hist(data_results$MS023_MMA_vs_GSK591_MMA_p.val)
hist(data_results$GSK591_MMA_vs_DMSO_MMA_p.adj)
hist(data_results$MS023_MMA_vs_DMSO_MMA_p.adj)
hist(data_results$MS023_MMA_vs_GSK591_MMA_p.adj)
...
Peptide_Modifcations_longer <- as.data.frame(Peptide_Modifications %>% select(-name) %>%
  pivot_longer(cols= DMSO_MMA_1:MS023_MMA_3, names_to = c('ID')))
...
Peptide_Modifcations_longer_sum <- aggregate(Peptide_Modifcations_longer[grep('DMSO_MMA_1', colnames(Peptide_Modifcations_longer)):grep('MS023_MMA_3', colnames(Peptide_Modifcations_longer))], by=list(Gene.Symbol = Peptide_Modifcations_longer$Gene.Symbol, Description = Peptide_Modifcations_longer$Description, Accession = Peptide_Modifcations_longer$Accession, value = Peptide_Modifcations_longer$value), FUN=sum)
...
#write.csv(df_wide, 'Trypsin_MMA_perResidue_log2FC.csv')
...
hist(data_results$MS023_MMA_vs_GSK591_MMA_p.val)
hist(data_results$GSK591_MMA_vs_DMSO_MMA_p.adj)
hist(data_results$MS023_MMA_vs_DMSO_MMA_p.adj)d
hist(data_results$MS023_MMA_vs_GSK591_MMA_p.adj)
